...
var checkMarker = function checkMarker(projectName, todoName) {
  ...
  if (projectArray !== null) {
    ...
    if (projectArray[index].todo[todoIndex].status === false) {
      //change checkbox values true or falce checked or not checked depending on value of todo.status
      projectArray[index].todo[todoIndex].status = true;
      modifyCheckBoxCheck.checked = true;
      var nearestTodoDiv = document.querySelectorAll(".todoRenderDiv[data-todo-title-name = \"".concat(todoName, "\"] p"));
      nearestTodoDiv.forEach(function (item) {
        item.style.cssText = "text-decoration:line-through; text-decoration-thickness:4px;";
      });
      ...
      var _nearestTodoDiv = document.querySelectorAll(".todoRenderDiv[data-todo-title-name = \"".concat(todoName, "\"] p"));
      _nearestTodoDiv.forEach(function (item) {
...
var renderProjects = function renderProjects(storage) {
  var project = (0,_projectManager__WEBPACK_IMPORTED_MODULE_0__.projectManager)();
  var todo = (0,_todoManager__WEBPACK_IMPORTED_MODULE_4__.todoManager)();
  if (storage !== null) {
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            ...
            if (todoCheck.checked === true) {
              var nearestTodoDiv = document.querySelectorAll(".todoRenderDiv[data-todo-title-name = \"".concat(item.title, "\"] p"));
              nearestTodoDiv.forEach(function (item) {
                item.style.cssText = "text-decoration:line-through; text-decoration-thickness:4px;";
              });
            }
            if (todoCheck.checked === false) {
              var _nearestTodoDiv = document.querySelectorAll(".todoRenderDiv[data-todo-title-name = \"".concat(item.title, "\"] p"));
              _nearestTodoDiv.forEach(function (item) {
                item.style.cssText = "text-decoration:none;";
              });
            }
  ...
  projectsDiv.addEventListener('click', function (event) {
    ...
    if (event.target.className === 'todoDeleteButton') {
      if (true) {
        ...
        var nearestProjectNameData = event.target.closest(".individualProjectContainer").dataset.projectName;
        var nearestTitleData = event.target.closest('.todoRenderDiv').dataset.todoTitleName;
        todo.deleteTodo(nearestProjectNameData, nearestTitleData);
        (0,_homePage__WEBPACK_IMPORTED_MODULE_2__.homePageRender)();
      }
    }
  });
};
